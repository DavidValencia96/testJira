...
from flask import jsonify, request  # Importa `request` de Flask
...
def generar_reporte(proyecto):
    ...
    wb = Workbook()
    sheet = wb.active
    sheet.append(["Clave", "Resumen", "Estado", "Asignado a", "Creado", "story point", "HU padre", "issuetype"])
        ...
        while start_at < total or total == 0:
            ...
            try:
                ...
                for issue in json_response['issues']:
                    ...
                    sheet.append([key, summary, status, assignee_name, created, storyP, parent, issuetype])
    ...
    # Guardar el archivo Excel en el directorio data
    file_path = os.path.join('data', f"issues_{proyecto}_jira.xlsx")
    wb.save(file_path)

    # Devolver la ruta del archivo y el tiempo de ejecución
    return {"archivo": file_path, "tiempo": execution_time}
